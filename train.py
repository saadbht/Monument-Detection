...
try:
    ...
    logger.warning('`test.py` not found in the project. mAP evaluation will be skipped.')
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP (if test.py exists)
                if test is not None:
                    ...
                    logger.warning('Skipping mAP evaluation because `test.py` is not present in this project.')
                    results = (0, 0, 0, 0, 0, 0, 0)  # P, R, mAP@.5, mAP@.5-.95, val_loss box/obj/cls
                    maps = np.zeros(nc)
                    times = (0, 0, 0)
